namespace FiguresLab
{
    ...
    public class Triangle : Figure, IFigure
    {
        ...
        public float getSquare()
        {
            if (_square == 0)
            {
                ...
                else throw new ArgumentException("Длины сторон должны иметь значение");
            }
            return _square;
        }
    ...
    {
        public static IFigure createCircle(float radius)
        {
            return new Circle(radius);
        }

        public static IFigure createTriangle(float[] sides)
        {
            return new Triangle(sides);
        }

        public static IFigure createTriangle(float side1, float side2, float side3)
        {
